...
public class Main {
    public static void main(String[] args) {
        ...
        for (Book book : bookList) {
            if(book.getCategory().equals("여행")){
                System.out.println("카테고리가 여행인 책 제목: " + book.getBookName());
            }
        }
        System.out.println("-".repeat(200));
        ...
        for (Book book : bookList) {
            if(book.getPrice()<=16200){
                System.out.println("가격 16200원 이하 책 제목: " + book.getBookName());
            }
        }
        System.out.println("-".repeat(200));
        ...
        for (Book book : bookList) {
            if(book.getBookName().contains("경제")){
                System.out.println("책 제목에 '경제'가 포함된 책 제목: " + book.getBookName());
            }
        }
        System.out.println("-".repeat(200));
        ...
        for (int i = 0; i < bookList.size(); i++) {
            if(maxPrice2<bookList.get(i).getPrice()){
                maxPrice2=bookList.get(i).getPrice();
            }
        }
        System.out.println("책 목록 중 가장 비싼 금액: " +maxPrice2);
        System.out.println("-".repeat(200));
        ...
        double sum2 = 0;
        for (int i = 0; i < bookList.size(); i++) {
            if (bookList.get(i).getCategory().equals("IT")) {
                sum2 += bookList.get(i).getPrice();
            }
        }
        System.out.println("카테고리 IT 책들의 가격 합: " + sum2);
        System.out.println("-".repeat(200));
...
//                .peek(book -> book.setPrice(book.getPrice() * 0.6)).toList();
        ...
        List<Book> discountedBookList2 = new ArrayList<>();
        for (Book book : bookList) {
            if (book.getCategory().equals("IT")) {
                discountedBookList2.add(book);
            }
        }
        for (Book book : discountedBookList2) {
            System.out.println("할인된 책 제목: " + book.getBookName());
            System.out.println("할인된 책 가격: " + book.getPrice());
        }
        System.out.println("-".repeat(200));
